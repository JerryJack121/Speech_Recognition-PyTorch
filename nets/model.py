...
class CNNLayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.transpose(2,
                           3).contiguous()  # (batch, channel, feature, time)
...
class ResidualCNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel, stride, dropout,
        ...
        self.cnn1 = nn.Conv2d(in_channels,
                              out_channels,
                              kernel,
                              stride,
                              padding=kernel // 2)
        self.cnn2 = nn.Conv2d(out_channels,
                              out_channels,
                              kernel,
                              stride,
                              padding=kernel // 2)
...
class SpeechRecognitionModel(nn.Module):
    ...
    def __init__(self,
        ...
        n_feats = n_feats // 2
        self.cnn = nn.Conv2d(1, 32, 3, stride=stride, padding=3 //
                             2)  # cnn for extracting heirachal features
...
if __name__ == "__main__":    
    #畫出模型架構
    model = SpeechRecognitionModel().cuda()
    summary(model, (1,128,2401))
